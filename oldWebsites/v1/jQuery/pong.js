$(document).ready(function() {
	...
	function reInit(){
		...
		clearInterval(game_loop);
		intervalTime=60;
		game_loop = setInterval(paint,intervalTime)
	}
	$('#pongDiv').fadeTo('slow',.3);
	...
	function moveBallKeyDown(){
		if(UP == true && paddle2.y<=(height/10)-4){
			removeOldPaddles();
			paddle2.y-=1;
			paintPaddles();
		} else if(paddle2.y>=(height/10)-4){
			//alert(paddle2.y);
			paddle2.y = (height/10)-4;
		}
		if(DOWN == true &&paddle2.y>=0){
			removeOldPaddles();
			paddle2.y+=1;
			paintPaddles();
		} else if (paddle2.y <=0){
	...
	function checkBallPos(){
		if(ball.y <= .2 || ball.y>=((height/10)-1)){
			if(bounceCheck == false){ //makes it so it doesn't get stuck on boundaries
				ball.speedY*=-1;
				bounceCheck = true;
			}
		} else {
